...
const CartPage: React.FC = () => {
      ...
      try {
        if (token && cartId) {
          ...
          for (const item of tempCart) {
            const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/products/${item.product_id}`);
            const data = await res.json();
            if (data && data._id) {
  ...
  const handleDeleteProduct = async (cartDetailId: string) => {
    ...
    try {
      const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/cart-details/${cartDetailId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleQuantityChange = async (id: string, newQty: number) => {
    ...
    try {
      const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/cart-details/${id}/quantity`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
