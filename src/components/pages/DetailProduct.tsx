...
const DetailProduct: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/reviews/product/${_id}`);
        const data = await res.json();
        if (data.success) {
  ...
  try {
    const res = await fetch('https://be-webdoluuniem.onrender.com/api/v1/reviews', {
      method: 'POST',
      headers: {
    ...
    if (data.success) {
      ...
      const res2 = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/reviews/product/${_id}`);
...
return (
  <div className="product-detail-container" style={{ background: '#fff', padding: 24, position: 'relative' }}>
    ...
    <div className="product-main" style={{ display: 'flex', gap: 24 }}>
      ...
      ? blog.image?.[0]
      : `https://be-webdoluuniem.onrender.com${blog.image?.[0]}`} alt="" style={{ width: 48, height: 48, background: '#ddd' }} />
              <div>{blog.title}</div>
            </div>
          </Link>
        ))}
