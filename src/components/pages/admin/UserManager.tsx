...
const UserManagement = () => {
  ...
  const handleDeleteUser = async (userId: string) => {
    ...
    try {
      const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/users/${userId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleSubmitEdit = async () => {
    ...
    try {
      const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/users/${editingUser._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
