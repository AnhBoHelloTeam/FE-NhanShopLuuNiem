...
const handleUpdateProduct = async () => {
  ...
  try {
    ...
    const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/blogs/${editingBlog._id}`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`
      },
...
const handleSaveNewBlog = async (newBlog: any) => {
  ...
  const handleDeleteBlog = async () => {
    ...
    try {
      const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/blogs/${pendingDelete}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  if (Array.isArray(blog.image) && blog.image.length > 0) {
    ...
    if (typeof rawImage === 'string') {
      if (rawImage.startsWith('http') || rawImage.startsWith('data:image')) {
        ...
        imageSrc = `https://be-webdoluuniem.onrender.com${rawImage}`;
      }
    }
  }
  return (
