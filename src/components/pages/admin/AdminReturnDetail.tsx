...
const AdminReturnDetail: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReturnDetail = async () => {
      try {
        ...
        const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/returns/${returnId}`, {
          method: 'GET',
          headers: {
  ...
  const handleApproveReturn = async () => {
    try {
      const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/returns/${returnId}/approve`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  const handleRejectReturn = async () => {
    try {
      const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/returns/${returnId}/reject`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {returnRequest.images && returnRequest.images.length > 0 ? (
              returnRequest.images.map((image: string, index: number) => (
                <img
                  key={index}
                  src={`https://be-webdoluuniem.onrender.com${image}`}
                  alt={`Product ${index + 1}`}
