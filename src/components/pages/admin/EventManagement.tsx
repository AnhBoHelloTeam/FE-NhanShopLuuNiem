...
const handleUpdateEvent = async (editEvent: any) => {
  ...
  try {
    ...
    const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/events/${editEvent._id}`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/events/${pendingDelete}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleAddProductToEvent = async (eventId: string) => {
    ...
    try {
      const res = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/events/${eventId}/add-products`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      <div key={index} style={{ position: 'relative' }}>
        <img 
          src={    
            ...
            img.startsWith('blob')
          ? img
          : `https://be-webdoluuniem.onrender.com${img}`}
          alt={`Ảnh ${index + 1}`}
          style={{
            width: '80px',
            height: '80px',
...
const ImageSlider = ({ images }: { images: string[] }) => {
  ...
  const getImageSrc = (imgPath: string) => {
    ...
    return `https://be-webdoluuniem.onrender.com${imgPath.startsWith('/') ? imgPath : '/' + imgPath}`;
  };
