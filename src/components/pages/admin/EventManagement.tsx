...
// hàm lưu chỉnh sửa 
  ...
  try {
    const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/events/${editingEvent._id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
...
const handleDeleteEvent = async (eventId: string) => {
  ...
  try {
    const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/events/${eventId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`
