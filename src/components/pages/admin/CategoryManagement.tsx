...
const CategoryManagement = () => {
  ...
  const handleUpdateProduct = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/categories/${editingCategory._id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`
...
const handleDeleteCategory = async () => {
  ...
  try {
    const response = await fetch(`https://be-webdoluuniem.onrender.com/api/v1/categories/${pendingDelete}`, {
      method: "DELETE",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
          ...
          <div style={{ position: "relative" }}>
            <img
              src={ 
                ...
                ? editingCategory.image 
                : `https://be-webdoluuniem.onrender.com${editingCategory.image}`
              }
              alt="Ảnh"
              style={{
                width: "80px",
          ...
          <div style={{ position: "relative" }}>
            <img
              src={ addingCategory.image.startsWith('http') ||
                ...
                ? addingCategory.image 
                : `https://be-webdoluuniem.onrender.com${addingCategory.image}`}
              alt="Ảnh"
              style={{
                width: "80px",
                height: "80px",
  ...
  if (category.image) {
    if (typeof category.image === 'string') {
      if (category.image.startsWith('http') || category.image.startsWith('data:image')) {
        ...
        imageSrc = `https://be-webdoluuniem.onrender.com${category.image}`;
      }
    }
  }
